...
func julianDay(t time.Time) C.double {
	t1 := t.UTC()
	...
	return C.ln_get_julian_day(&lnd)
}

func GeocentricCoordinates(cb Body, t time.Time) (LLACoords, error) {
	cj := julianDay(t)
	...
	default:
		return LLACoords{}, fmt.Errorf("unknown celestial body")
	}
